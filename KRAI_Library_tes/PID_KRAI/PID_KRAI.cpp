...
void PID::onlineTuning(float setpoint, float feedback)
{
    ...
    if (!rise_is_reached && (abs(feedback) > 0.7 * abs(setpoint)))
    {
        rise_is_reached = 1;
    ...
    if ((prevsp != 0) && (prevsp != setpoint))
    {
        ...
        else
        {
            ...
            else if ((rise_time > 2000) || (float(rise_time) / prev_rise_time > 1.5) || !rise_is_reached)
            {
                kp = 1.2 * kp;
            }
        }

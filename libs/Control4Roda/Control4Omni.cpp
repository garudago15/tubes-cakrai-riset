...
void Control4Omni::encoderMotorSamp(){
    this->baseSpeed();
    this->v_FL_curr = (float)this->encFL->getPulses()*2*PI*WHEEL_RAD*S_TO_US/(WHEEL_PPR*TIME_SAMPLING);
    this->v_FR_curr = (float)this->encFR->getPulses()*2*PI*WHEEL_RAD*S_TO_US/(WHEEL_PPR*TIME_SAMPLING);
    this->v_BR_curr = (float)this->encBR->getPulses()*2*PI*WHEEL_RAD*S_TO_US/(WHEEL_PPR*TIME_SAMPLING);
    this->v_BL_curr = (float)this->encBL->getPulses()*2*PI*WHEEL_RAD*S_TO_US/(WHEEL_PPR*TIME_SAMPLING);
...
void Control4Omni::base() {
    ...
    if(fabs(this->vy_motor - this->vy_last) > MAX_ACCEL_Y*TIME_SAMPLING/S_TO_US){
        if (this->vy_last > this->vy_motor){
            this->vy_motor = this->vy_last - MAX_ACCEL_Y*TIME_SAMPLING/S_TO_US;
        } else {
            this->vy_motor = this->vy_last + MAX_ACCEL_Y*TIME_SAMPLING/S_TO_US;
        }
    }
    if(fabs(this->vx_motor - this->vx_last) > MAX_ACCEL_X*TIME_SAMPLING/S_TO_US){
        if (this->vx_last > this->vx_motor){
            this->vx_motor = this->vx_last - MAX_ACCEL_X*TIME_SAMPLING/S_TO_US;    
        }
        else{
            this->vx_motor = this->vx_last + MAX_ACCEL_X*TIME_SAMPLING/S_TO_US;
        }
...
bool Control4Omni::isMoving() {
    if(this->v_FL_curr==float(0) && this->v_FR_curr==float(0) && this->v_BR_curr==float(0) && this->v_BL_curr==float(0)){
        return false;
    } else{
        return true;
    }
